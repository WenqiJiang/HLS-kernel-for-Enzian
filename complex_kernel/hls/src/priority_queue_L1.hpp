...
class Priority_queue_L1;

template<const int queue_size> 
class Priority_queue_L1<PQ_out_t, queue_size, Collect_smallest> {
...
#pragma HLS pipeline II=1
    ...
    private:
        ...
        void compare_swap(PQ_out_t* array, int idxA, int idxB) {
            // if smaller -> swap to right
            // note: idxA must < idxB
#pragma HLS inline
            if (array[idxA].dist < array[idxB].dist) {
                PQ_out_t regA = array[idxA];
                PQ_out_t regB = array[idxB];
                array[idxA] = regB;
                array[idxB] = regA;
            }
        }

        void compare_swap_array_step_A(PQ_out_t* array) {
...
#pragma HLS UNROLL
        ...
        void compare_swap_array_step_B(PQ_out_t* array) {
